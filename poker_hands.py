...
def deal_with_draws(draw_hands):
    ...
    if len(winners_second_pass) == 1:
        return construct_win_msg(winners_second_pass[0],losers_first_pass[0],"win_1st_pass")
    else:
...
def deal_with_draws_2nd_pass(pass_hands):
    for hand_type in PHand:
        ...
        if len(winners_final_pass) == 1:
            return construct_win_msg(winners_final_pass[0], losers_second_pass[0], "win_2nd_pass")            
        else:
...
def sequential_card_check(seq_hands):
    for sequential_cards in range(0,5):
        if seq_hands[0].card_values[sequential_cards] != seq_hands[1].card_values[sequential_cards]:
            if seq_hands[0].card_values[sequential_cards] > seq_hands[1].card_values[sequential_cards]:
                return construct_win_msg(seq_hands[0],seq_hands[1],"win_seq",sequential_cards)
            elif seq_hands[0].card_values[sequential_cards] < seq_hands[0].card_values[sequential_cards]:
                return construct_win_msg(seq_hands[1],seq_hands[0],"win_seq",sequential_cards)  
    return "Tie"    
...
def construct_win_msg(winner, loser, win_type, card_index=-1):
    ...
    if win_type == "win":
        ...
        return base_str + ": " + str(winner.highest_card_in_tie_breaker) + " over " + str(loser.highest_card_in_tie_breaker)
    elif win_type == "win_2nd_pass":
        return (base_str
            + ": " + str(winner.remainder_high_card_after_high_hand(winner.dict_high_card_lookup.get(winner.phand)))
            + " over " + str(loser.remainder_high_card_after_high_hand(loser.dict_high_card_lookup.get(loser.phand))))
