...
class Hand:
    ...
    def __init__(self, raw_card_input, player_colour):
        ...
        self.highest_card_in_tie_breaker = self.highest_card_for_tie_breaker(
            self.dict_high_card_lookup.get(self.phand))
        self.remainder_hand_after_high_hand\
            = self.remainder_hand_after_highest_poker_hand()
    ...
    def remainder_hand_after_highest_poker_hand(self):
        ...
        while r_item in self.remainder_values:
            self.remainder_values.remove(r_item)
        unique_values = {}
        ...
        if unique_values[max(unique_values, key=unique_values.get)] == 2:
            self.highest_card_of_remaining_hand =\
                self.remainder_high_card_after_high_hand(2)
            return PHand.A_PAIR.value
        else:
            self.highest_card_of_remaining_hand =\
                self.remainder_high_card_after_high_hand(1)
            return PHand.HIGH_CARD.value
